...
if (isset($_POST) && $_POST) { //O isset é para verificar se, além de existir, ele tem algum valor
  ...
  //fazendo agora é basicamente verificar se há POST e, se houver, se há $resultado.


  $base = $_POST["base"];
  $altura = $_POST["altura"];

  // primeiro atribuiu o POST à uma variável e depois usou a variável para atribuir um valor 
  // às propriedades do objeto.
  // fizemos agora foi definir PROPRIEDADES (ou atributos, características) que os objetos receberão 
  // ao serem instanciados a partir da classe Retangulo.

  // Instanciando um objeto a partir da classe Retangulo 
  $retangulo = new Retangulo();
  // TODO objeto instanciado a partir dessa classe precisará de $base e $altura

  $retangulo->baseProp = $base;
  $retangulo->alturaProp = $altura;
  // Ao instanciar um objeto a partir de uma classe, precisamos declarar os valores das 
  //propriedades declaradas dentro da classe.
  //E sempre conseguiremos acessar propriedades (definidas na classe, se houver)
  // e métodos declarados na classe.
...
?>
<!-- Resumo -->
<!-- // Acionando o arquivo da classe Retangulo     
require_once("./classes/retangulo.php");      
// Criando condição que verifica se recebemos valores por POST     
if(isset($_POST) && $_POST):          
// Instanciando um objeto a partir da classe Retangulo         
$retangulo = new Retangulo();          
// Atribuindo os valores recebidos por $_POST         
$retangulo->baseProp = $_POST["base"];         
$retangulo->alturaProp = $_POST["altura"];          
// Chamando o método (função) calcularArea()         
